...
struct DetailHotelView: View {
    ...
    var body: some View {
        ...
        ScrollView{
            ...
            VStack{
                Image(item_h.image).resizable().frame(height: 280)
            }
            
            VStack{
                Text(item_h.name).fontWeight(.regular)
            ...
            HStack{
                ...
                Text(item_h.price).fontWeight(.regular)
            ...
            VStack{
                ...
                Text(item_h.description).padding(.horizontal)
            }
            ...
            VStack{
                ...
                ScrollView(.horizontal){
                    HStack{
                        ForEach(item_h.list, id: \.self) { l in
            ...
            VStack{
                HStack{
                    Text("Opinie").fontWeight(.regular)
                        .font(.custom("Helvetica Neue", size: 20)).padding()
                    Text(String(item_h.opinion) + " / 5.0").fontWeight(.regular)
                        .font(.custom("Helvetica Neue", size: 18)).padding()
                Spacer()
                }
                VStack{
                    ProgressBarView(width: screenWidth, height: 30, percent: CGFloat(item_h.opinion) * 20, c1: Color("C2"), c2: Color("C1"))}
                .padding(20)
            }
...
struct DetailHotelView_Previews: PreviewProvider {
    static var previews: some View {
        DetailHotelView(item_h: hotels[1])
    }
}
